                        ...
                        @elseif ($tipe1 === 'upload')
                            ...
                            <div class="mb-3">
                                ...
                                <select name="tipe_file" class="form-control">
                                    @foreach(['word', 'excel', 'pdf', 'image', 'video'] as $tipe1File)
                                        <option value="{{ $tipe1File }}" {{ $data->tipe_file == $tipe1File ? 'selected' : '' }}>
                                            {{ ucfirst($tipe1File) }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="deskripsi_upload" id="deskripsi_upload" class="form-control mt-3"
                                    placeholder="Deskripsi singkat...">{{ old('deskripsi_upload', $data->deskripsi) }}</textarea>

                                <label class="mt-3">Instruksi Lengkap (Editor)</label>
                                <div id="upload-editor" style="height: 150px; background-color: white;">
                                    {!! old('konten_upload', $data->konten) !!}
                                </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            if (tipe === 'materi') {
                ...
                const deskripsi = document.getElementById('deskripsi_upload');

                // Quill editor untuk instruksi lengkap
                const quillUpload = new Quill('#upload-editor', {
                    theme: 'snow',
                ...
                quillUpload.on('text-change', updatePreview);
