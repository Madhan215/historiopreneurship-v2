...
@section('container-content')
    ...
    <p class="mt-2">
        Berdasarkan materi yang disampaikan oleh pakar, diskusikanlah materi tersebut dengan menguraikan perspektif kalian
        dalam bentuk ringkasan dan peta konsep terkait pemasaran kewirausahaan kesejarahan!
    </p>
    <p>
    ...
    <form action="{{ route('simpanAktivitas') }}" method="POST">
        ...
        <textarea class="form-control w-100 my-2" name="jawaban[analisaKelompok]" id="analisaKelompok" rows="5"
            placeholder="Tuliskan hasil analisa kelompok disini">{{ old('jawaban.analisaKelompok', $jawabanKelompok->where('kategori', 'aktivitas 3')->where('aspek', 'Hasil analisa kelompok')->first()->jawaban ?? '') }}</textarea>
    ...
    <ol>
        <li>
            <b>KATEGORI 1</b><br>Jika hasil analisa mengambarkan jawaban yang tidak lengkap, tidak terstruktur dan tidak
            tepat sasaran
        </li>
        <li class="mt-3">
            <b>KATEGORI 2</b><br>Jika hasil analisa mengambarkan jawaban yang cukup lengkap, cukup terstruktur dan cukup
            tepat sasaran
        </li>
        <li class="mt-3">
        ...
        <li class="mt-3">
            <b>KATEGORI 4</b><br>Jika hasil analisa mengambarkan jawaban yang sangat lengkap, sangat terstruktur dan sangat
            tepat sasaran
        </li>
    </ol>

    @if(
            !empty($jawabanKelompok->where('kategori', 'aktivitas 3')->where('aspek', 'Hasil analisa kelompok')->first()?->jawaban) && $user->peran ==='guru'
        )
        <div class="card-body mt-3">
            ...
            <div class="input-group mb-3">
                ...
                <input type="number" class="form-control" name="nilai_akhir" min="0" max="100"
                    value="{{ $nilaiKelompokAktivitas3->nilai_akhir ?? '' }}" readonly>
            </div>
            ...
            <textarea class="form-control w-100 mt-2" name="data_jawaban_penilai" id="feedbackIndividu" rows="5"
                readonly>{{ $nilaiKelompokAktivitas3->data_jawaban_penilai ?? '' }}</textarea>
        </div>
    @endif
